# SOURCES.md

## AI Tools

### Claude


Used for debugging and understanding Pyro5 wire protocols.

Specific uses:

-> Wire protocal header format: Asked claude to help with interpreting the byte-by-byte caputers on Wireshark, and help with the field layout and broke down what means what. 

->Debugging service discovery. Just went over my code and show the errors such as not importing 'pyro_message.py correctly etc. 

All code was reviewed and tested. before submission it was modified by me before submission. It was not copied and pasted. 